/**
 * =========================================================================================
 * Project     : ESP32 Real-Time 10-Band I2S Audio DSP Parametric & Graphic Equalizer
 * Platform    : ESP32 (Dual-Core Xtensa LX6 / FreeRTOS)
 * Framework   : Arduino IDE 2.0+
 * Author      : Muhammad Fikri
 * License     : MIT
 * Description : High-fidelity digital signal processor reading stereo 24-bit/48kHz I2S audio, calculating 10-band biquad IIR filters in parallel, with web UI curve manipulation.
 * =========================================================================================
 */
...
void TaskTelemetryBroadcaster(void *pvParameters) {
    ...
    for (;;) {
        if (xSemaphoreTake(g_telemetryMutex, pdMS_TO_TICKS(20)) == pdTRUE) {
            ...
            doc["system"] = "esp32-i2s-dsp-graphic-equalizer";
...
void setup() {
    ...
    Serial.println(F("================================================================="));
    Serial.println(F(" ESP32 Real-Time 10-Band I2S Audio DSP Parametric & Graphic Equalizer"));
